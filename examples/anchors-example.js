...
export default async function main() {
  ...
  const anchorResult = await myChannel.anchor(
    Buffer.from("hello, world 1"),
    myChannel.firstAnchorageID
  );
  console.log("Message ID: ", anchorResult.msgID);

  console.log("Anchoring a message to ", anchorResult.msgID, "...");
  const anchorResult2 = await myChannel.anchor(
  ...
  const fetchResult = await myChannel.fetch(
    myChannel.firstAnchorageID,
    anchorResult.msgID
  );
  ...
  const fetchResult2 = await myChannel.fetch(
    anchorResult.msgID,
    anchorResult2.msgID
  );
